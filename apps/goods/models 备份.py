...
class GoodsType(BaseModel):
    '''商品类型模型类'''
    name = CharField(max_length=20, verbose_name='商品种类')  # 比如：直下式,侧入式
    logo = CharField(max_length=20, verbose_name='标识')  # meta
    image = ImageField(upload_to='type', verbose_name='商品类型图片')  # 调用FastDfs类获得url
...
class Goods(BaseModel):
    '''商品SPU模型类'''
    name = CharField(max_length=20, verbose_name='商品型号:JF-D-002')
    # 富文本类型:带有格式的文本
    # detail = HTMLField(blank=True, verbose_name='商品详情')
    # 下面是不限制字符的文本
    listing = TextField(blank=True, verbose_name="listing")
